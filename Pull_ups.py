...
cap = cv2.VideoCapture("Pulls.mp4")
...
while True:
    ...
    # print(lmList)
    if len(lmList) != 0:
        angleL=angle = detector.findAngle(img, 13, 11, 23) #Left Arm
        angleR=angle = detector.findAngle(img, 14, 12, 24) #Right Arm
        # angle = detector.findAngle(img, 23, 25, 27) #Left Leg
        # angle = detector.findAngle(img, 24, 26, 28) #Right Leg
        per = np.interp((angleL+angleR)/2, (70, 140), (0, 100))
        ...
        if per==0:
            if count%4==0:
                cv2.putText(img, str("Keep the Fire Alive !!!"), (450, 75), cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 5)
            elif count%3==0:
                cv2.putText(img, str("Great !!!"), (450, 75), cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 5)
            elif count%2==0:
                cv2.putText(img, str("Nice!!!"), (450, 75), cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 5)
            else:
